...
def delete_node(conn: sqlite3.Connection, node_id: int) -> None:
    """Delete one node and every row referencing it.

    The edges/file_ownership FKs do not declare ON DELETE CASCADE, so with
    foreign_keys=ON the dependent rows must be removed first or the node delete
    raises a constraint error. Callers handle child/subtree policy.
    """
