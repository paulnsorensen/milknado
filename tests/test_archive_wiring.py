...
class _RebalanceResponse(TypedDict):
...
def _call(tool: Callable[..., _T], **kwargs: object) -> _T:
    """Invoke a FastMCP tool through its underlying Python callable."""
    fn = getattr(tool, "fn", tool)
    return fn(**kwargs)


def _call_rebalance(**kwargs: object) -> _RebalanceResponse:
    # TODO(milknado): promote response TypedDicts into mcp/_core.py
    return cast(_RebalanceResponse, cast(object, _call(milknado_rebalance, **kwargs)))


def _call_get_node(**kwargs: object) -> _NodeDetails:
    # TODO(milknado): promote response TypedDicts into mcp/_core.py
    return cast(_NodeDetails, cast(object, _call(milknado_get_node, **kwargs)))
...
def _seed_done_subtree(root: str) -> tuple[int, int]:
    ...
    _ = _call(milknado_todo_set_status, node_id=goal["id"], status="done", project_root=root)
...
class TestMcpIncludeArchived:
    ...
    def test_set_subtree_status_skips_archived(self, tmp_path: Path) -> None:
        ...
        _ = _call(
            milknado_todo_set_status, node_id=shelved["id"], status="done", project_root=root
        )
        _ = _call(milknado_archive_node, node_id=shelved["id"], project_root=root)
        ...
        full = _call(milknado_todo_tree, project_root=root, include_archived=True)
        by_id = {n["id"]: n for n in full[0].get("children", [])}
...
class TestMcpSetStatusGuard:
    ...
    def test_status_change_on_archived_node_raises_and_leaves_it_untouched(
        ...
        with pytest.raises(ValueError, match="is archived"):
            _ = _call(
                milknado_todo_set_status, node_id=task_id, status="blocked", project_root=root
            )
...
class TestMcpRebalanceRestructure:
    ...
    def test_orphans_grouped_under_find_or_create_inbox_and_inbox_reused(
        ...
        result = _call_rebalance(project_root=root, reap=False)
        ...
        assert {c["id"] for c in tree[0].get("children", [])} == {first["id"], second["id"]}

        # A second run find-or-creates nothing: the Inbox is reused, no orphans remain.
        again = _call_rebalance(project_root=root, reap=False)
    ...
    def test_diamond_orphans_move_exactly_once_under_inbox(self, tmp_path: Path) -> None:
        ...
        for task in (shared, left, right):
            assert _call_get_node(node_id=task["id"], project_root=root)["parent_id"] == inbox_id

        def _count(node: NodeSummary, target: int) -> int:
            children = node.get("children", [])
            return (node["id"] == target) + sum(_count(c, target) for c in children)

        tree = _call(milknado_todo_tree, project_root=root)
        inbox = next(n for n in tree if n["id"] == inbox_id)
        # Parentage: each orphan is a direct Inbox child exactly once.
        assert sorted(c["id"] for c in inbox.get("children", [])) == sorted(
...
class TestMcpRebalance:
    def test_dry_run_predicts_without_mutating(self, tmp_path: Path) -> None:
        ...
        result = _call_rebalance(dry_run=True, project_root=root)
    ...
    def test_dry_run_on_uninitialized_project_creates_nothing(self, tmp_path: Path) -> None:
        result = _call_rebalance(dry_run=True, project_root=str(tmp_path))
...
class TestCliGraphArchive:
    ...
    def test_unarchive_under_archived_ancestor_refused(self, tmp_path: Path) -> None:
        ...
        _ = runner.invoke(app, ["graph", "archive", str(goal_id), "--project-root", str(tmp_path)])
        result = runner.invoke(
