# These tests intentionally inspect the graph's SQL seams.
...
def test_ready_nodes_reject_unbounded_limits(tmp_path: Path, limit: int) -> None:
    ...
    with pytest.raises(ValueError, match="limit must be between 1 and 100"):
        _ = graph.get_ready_nodes(limit=limit)
    graph.close()
...
def test_ready_nodes_filter_kind_and_flavor(tmp_path: Path) -> None:
    graph = MikadoGraph(tmp_path / "graph.db")
    goal = graph.add_node("goal", spec=NodeSpec(kind=NodeKind.GOAL))
    _ = graph.add_node("implement", parent_id=goal.id, spec=NodeSpec(flavor="implement"))
...
def test_node_summaries_reject_invalid_pages(
    ...
    with pytest.raises(ValueError, match=message):
        _ = graph.get_node_summaries(page=page)
    graph.close()
...
def test_opening_current_schema_installs_query_indexes(tmp_path: Path) -> None:
    ...
    _ = graph_conn(graph).execute("DROP INDEX idx_nodes_ready")
    _ = graph_conn(graph).execute("DROP INDEX idx_edges_child")
    ...
    node_indexes = cast(
        list[tuple[object, ...]],
        graph_conn(reopened).execute("PRAGMA index_list(nodes)").fetchall(),
    )
    edge_indexes = cast(
        list[tuple[object, ...]],
        graph_conn(reopened).execute("PRAGMA index_list(edges)").fetchall(),
    )
    indexes = {row[1] for row in node_indexes} | {row[1] for row in edge_indexes}
...
def test_bulk_node_and_latest_result_queries_preserve_contract(tmp_path: Path) -> None:
    ...
    graph.runs.start("run-first", first.id, "log", "2026-01-01T00:00:00+00:00", None)
    graph.runs.start("run-second", second.id, "log", "2026-01-01T00:00:00+00:00", None)
    _ = graph.runs.deposit_message("run-first", "result", "old", "2026-01-01T00:00:01+00:00")
    _ = graph.runs.deposit_message("run-first", "result", "new", "2026-01-01T00:00:02+00:00")
    _ = graph.runs.deposit_message("run-second", "result", "second", "2026-01-01T00:00:01+00:00")
