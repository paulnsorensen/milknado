...
# These tests intentionally exercise private batching and MCP seams.
...
class TestEstimateTokens:
    def test_zero_length_file_returns_zero(self, tmp_path: Path) -> None:
        f = tmp_path / "empty.py"
        _ = f.write_text("")
    ...
    def test_binary_file_does_not_crash(self, tmp_path: Path) -> None:
        ...
        _ = f.write_bytes(bytes(range(256)))
    ...
    def test_path_traversal_falls_back_gracefully(self, tmp_path: Path) -> None:
    ...
    def test_add_unknown_extension_uses_defaults(self, tmp_path: Path) -> None:
        # DEFAULT_LINES=150, DEFAULT_TPL=8, HEADROOM=1.25 -> 1500
        c = FileChange(id="1", path="config", edit_kind="add")
        result = estimate_tokens(c, tmp_path)
        assert result == 1500

    def test_double_extension_bak_unknown(self, tmp_path: Path) -> None:
        # ext='bak' not in tables -> DEFAULT
        c = FileChange(id="1", path="a.py.bak", edit_kind="add")
        result = estimate_tokens(c, tmp_path)
        assert result == 1500

    def test_invalid_edit_kind_does_not_crash(self, tmp_path: Path) -> None:
        # edit_kind='garbage' not in FLAT_COST, not 'modify' -> falls to add path
        c = FileChange(id="1", path="a.py", edit_kind="garbage")  # pyright: ignore[reportArgumentType]  # type: ignore
        result = estimate_tokens(c, tmp_path)
        assert isinstance(result, int)
        assert result > 0
...
class TestParseImpactDict:
    _ca: FileChange = FileChange(id="id_a", path="a.py")
    _cb: FileChange = FileChange(id="id_b", path="b.py")
    _path_to_ids: dict[str, list[str]] = {"a.py": ["id_a"], "b.py": ["id_b"]}
    _id_to_change: dict[str, FileChange] = {"id_a": _ca, "id_b": _cb}
...
class TestBuildChangeGraph:
    ...
    def test_self_loop_new_relationship_filtered(self):
        ...
        _, edges, _ = build_change_graph([a], new_relationships=[rel])
    ...
    def test_duplicate_new_relationships_deduplicated(self):
        ...
        _, edges, _ = build_change_graph([a, b], new_relationships=[rel, rel])
    ...
    def test_unknown_new_relationship_endpoint_raises(self):
        ...
        with pytest.raises(ValueError, match="unknown edge endpoint"):
            _ = build_change_graph([a], new_relationships=[rel])
...
class TestPlanBatchesBoundary:
    def test_budget_zero_oversized(self, tmp_path: Path) -> None:
    ...
    def test_budget_negative_oversized(self, tmp_path: Path) -> None:
    ...
    def test_time_limit_zero_valid_status(self, tmp_path: Path) -> None:
    ...
    def test_time_limit_negative_rejected(self, tmp_path: Path) -> None:
        c = FileChange(id="1", path="a.py", edit_kind="delete")
        with pytest.raises(ValueError, match="time_limit_s"):
            _ = plan_batches([c], budget=70_000, time_limit_s=-1, root=tmp_path)

    def test_duplicate_paths_different_ids_ok(self, tmp_path: Path) -> None:
    ...
    def test_circular_depends_on_co_batched(self, tmp_path: Path) -> None:
    ...
    def test_budget_exact_fit(self, tmp_path: Path) -> None:
        c = FileChange(id="1", path="a.py", edit_kind="delete")  # exactly 80 tokens
        plan = plan_batches([c], budget=80, root=tmp_path)
        assert plan.solver_status in ("OPTIMAL", "FEASIBLE")

    def test_budget_one_under_oversized(self, tmp_path: Path) -> None:
    ...
    def test_split_symbol_spread_reported(self, tmp_path: Path) -> None:
    ...
    def test_200_changes_no_hang(self, tmp_path: Path) -> None:
...
class TestPlanBatchesImpl:
    ...
    def _stub_crg(self, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    def test_symbol_missing_name_raises(self, tmp_path: Path) -> None:
        from milknado.app.plan_batches import plan_batches as _plan_batches_impl
    ...
    def test_symbol_missing_file_raises(self, tmp_path: Path) -> None:
        from milknado.app.plan_batches import plan_batches as _plan_batches_impl
    ...
    def test_empty_changes_returns_optimal(self, tmp_path: Path) -> None:
        from milknado.app.plan_batches import plan_batches as _plan_batches_impl
