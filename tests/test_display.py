...
class TestSummarize:
    def test_empty_graph(self, graph: MikadoGraph):
        s = summarize(graph)
        assert s.total == 0
        assert s.done == 0
    ...
    def test_single_node(self, graph: MikadoGraph):
    ...
    def test_single_node_with_leaf_shows_leaf_ready(self, graph: MikadoGraph):
    ...
    def test_completion_percentage(self, graph: MikadoGraph):
        root = graph.add_node("Root")
        c1 = graph.add_node("Child 1", parent_id=root.id)
        _ = graph.add_node("Child 2", parent_id=root.id)
        graph.mark_running(c1.id)
        graph.mark_done(c1.id)
        s = summarize(graph)
        assert s.total == 3
        assert s.done == 1
        assert abs(s.pct_complete - 33.3) < 0.1

    def test_counts_by_status(self, graph: MikadoGraph):
        root = graph.add_node("Root")
        c1 = graph.add_node("Child 1", parent_id=root.id)
    ...
    def test_ready_excludes_non_pending(self, graph: MikadoGraph):
        root = graph.add_node("Root")
        c1 = graph.add_node("Child 1", parent_id=root.id)
        graph.mark_running(c1.id)
        s = summarize(graph)
    ...
    def test_conflicts_detected(self, graph: MikadoGraph):
        root = graph.add_node("Root")
        c1 = graph.add_node("Child 1", parent_id=root.id)
        c2 = graph.add_node("Child 2", parent_id=root.id)
    ...
    def test_no_conflicts_when_disjoint(self, graph: MikadoGraph):
        root = graph.add_node("Root")
        c1 = graph.add_node("Child 1", parent_id=root.id)
        c2 = graph.add_node("Child 2", parent_id=root.id)
...
class TestRenderTree:
    ...
    def test_single_root(self, graph: MikadoGraph):
        _ = graph.add_node("My root goal")
        output = render_tree(graph)
        assert "My root goal" in output
        assert "0%" in output or "0/" in output

    def test_tree_structure(self, graph: MikadoGraph):
        root = graph.add_node("Root")
        _ = graph.add_node("Child A", parent_id=root.id)
        _ = graph.add_node("Child B", parent_id=root.id)
        output = render_tree(graph)
        assert "Root" in output
        assert "Child A" in output
        assert "Child B" in output

    def test_nested_tree(self, graph: MikadoGraph):
        root = graph.add_node("Root")
        mid = graph.add_node("Middle", parent_id=root.id)
        _ = graph.add_node("Leaf", parent_id=mid.id)
        output = render_tree(graph)
        assert "Root" in output
        assert "Middle" in output
        assert "Leaf" in output

    def test_done_nodes_in_output(self, graph: MikadoGraph):
        root = graph.add_node("Root")
    ...
    def test_ready_nodes_listed(self, graph: MikadoGraph):
        root = graph.add_node("Root")
        _ = graph.add_node("Ready leaf", parent_id=root.id)
        output = render_tree(graph)
        assert "Ready" in output
        assert "Ready leaf" in output

    def test_conflicts_displayed(self, graph: MikadoGraph):
        root = graph.add_node("Root")
    ...
    def test_running_shows_worktree(self, graph: MikadoGraph):
        root = graph.add_node("Root")
    ...
    def test_active_worktrees_section(self, graph: MikadoGraph):
        root = graph.add_node("Root")
    ...
    def test_no_active_worktrees_when_none_running(self, graph: MikadoGraph):
    ...
    def test_run_state_unknown_when_no_run_states(self, graph: MikadoGraph):
        root = graph.add_node("Root")
        c1 = graph.add_node("Worker", parent_id=root.id)
        graph.mark_running(c1.id, worktree_path="/tmp/wt-1", run_id="run-abc")
        output = render_tree(graph)
...
class TestSummarizeActiveWorktrees:
    def test_active_worktrees_populated(self, graph: MikadoGraph):
        root = graph.add_node("Root")
    ...
    def test_no_active_worktrees_when_pending(self, graph: MikadoGraph):
        _ = graph.add_node("Root")
        s = summarize(graph)
        assert s.active_worktrees == []

    def test_running_without_worktree_excluded(self, graph: MikadoGraph):
