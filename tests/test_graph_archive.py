...
# These tests intentionally exercise private graph mutation seams.
...
class TestArchiveSubtree:
    def test_all_done_subtree_stamps_every_node_and_returns_count(
        ...
        assert _mutations.archive_subtree(graph_conn(graph), goal.id, now=STAMP) == 3
    ...
    def test_defaults_to_utcnow_stamp(self, graph: MikadoGraph) -> None:
        ...
        assert _mutations.archive_subtree(graph_conn(graph), node.id) == 1
    ...
    def test_mixed_status_subtree_raises_naming_ids_and_stamps_nothing(
        ...
        with pytest.raises(ArchiveIneligible) as excinfo:
            _ = _mutations.archive_subtree(graph_conn(graph), goal.id)
    ...
    def test_rearchive_is_idempotent_and_restamps(self, graph: MikadoGraph) -> None:
        ...
        assert _mutations.archive_subtree(graph_conn(graph), goal.id, now=later) == 3
        for nid in (goal.id, left.id, right.id):
    ...
    def test_archive_subtree_containing_already_archived_descendant_restamps_all(
        ...
        goal, left, right = _done_goal_with_two_tasks(graph)
        _ = _mutations.archive_subtree(graph_conn(graph), left.id, now=STAMP)
        later = "2026-07-24T01:00:00+00:00"
        assert _mutations.archive_subtree(graph_conn(graph), goal.id, now=later) == 3
        for nid in (goal.id, left.id, right.id):
            assert _node(graph, nid).archived_at == datetime.fromisoformat(later)
...
class TestUnarchiveSubtree:
    def test_unarchive_from_archived_root_clears_whole_subtree(self, graph: MikadoGraph) -> None:
        goal, left, right = _done_goal_with_two_tasks(graph)
        _ = _mutations.archive_subtree(graph_conn(graph), goal.id)
        assert _mutations.unarchive_subtree(graph_conn(graph), goal.id) == 3
        for nid in (goal.id, left.id, right.id):
            assert _node(graph, nid).archived_at is None
    ...
    def test_unarchive_under_archived_ancestor_refuses_naming_ancestor(
        ...
        with pytest.raises(
            ...
            _ = _mutations.unarchive_subtree(graph_conn(graph), right.id)
        for nid in (goal.id, left.id, right.id):
    ...
    def test_unarchive_deep_node_without_archived_ancestor_clears_only_archived_descendants(
        ...
        _ = _mutations.archive_subtree(graph_conn(graph), archived_leaf.id, now=STAMP)
        assert _mutations.unarchive_subtree(graph_conn(graph), mid.id) == 1
    ...
    def test_unarchive_partially_archived_subtree_clears_only_stamped_nodes(
        ...
        assert _mutations.unarchive_subtree(graph_conn(graph), goal.id) == 2
    ...
    def test_unarchive_sibling_subtree_leaves_other_archived_subtrees(
        ...
        for nid in (child_a.id, root_a.id, root_b.id):
            _complete(graph, nid)
        _ = _mutations.archive_subtree(graph_conn(graph), root_a.id)
        _ = _mutations.archive_subtree(graph_conn(graph), root_b.id)
        assert _mutations.unarchive_subtree(graph_conn(graph), root_a.id) == 2
    ...
    def test_unarchive_non_archived_node_returns_zero(self, graph: MikadoGraph) -> None:
        ...
        assert _mutations.unarchive_subtree(graph_conn(graph), node.id) == 0
        assert _node(graph, node.id).archived_at is None
    ...
    def test_unarchive_nested_shelves_from_outer_root_clears_everything(
        ...
        _ = _mutations.archive_subtree(graph_conn(graph), left.id)
        _ = _mutations.archive_subtree(graph_conn(graph), goal.id)
        ...
        with pytest.raises(ValueError, match="under archived ancestor"):
            _ = _mutations.unarchive_subtree(graph_conn(graph), grandchild.id)
        # The archived root owns the shelf; clearing from there restores all.
        assert _mutations.unarchive_subtree(graph_conn(graph), goal.id) == 4
    ...
    def test_unarchive_refuses_under_archived_edge_parent_in_diamond(
        ...
        p = graph.add_node("p")
        _ = graph.add_edge(p.id, x.id)
        for nid in (x.id, g.id, p.id):
            _complete(graph, nid)
        assert _mutations.archive_subtree(graph_conn(graph), p.id) == 2
        with pytest.raises(
            ValueError,
            match=(
            ...
            _ = _mutations.unarchive_subtree(graph_conn(graph), x.id)
        ...
        assert _mutations.unarchive_subtree(graph_conn(graph), p.id) == 2
    ...
    def test_unarchive_refuses_shared_descendant_owned_by_other_archive(
        ...
        _ = _mutations.archive_subtree(graph_conn(graph), left.id)
        _ = _mutations.archive_subtree(graph_conn(graph), right.id)

        with pytest.raises(ValueError, match=f"archived ancestor {right.id}"):
            _ = _mutations.unarchive_subtree(graph_conn(graph), left.id)
    ...
    def test_unarchive_missing_node_raises(self, graph: MikadoGraph) -> None:
        with pytest.raises(ValueError, match="Node 999 not found"):
            _ = _mutations.unarchive_subtree(graph_conn(graph), 999)
...
class TestSetSubtreeStatusSkipsArchived:
    def test_archived_nodes_are_skipped_not_stamped_not_errored(self, graph: MikadoGraph) -> None:
        ...
        _ = _mutations.archive_subtree(graph_conn(graph), shelved_task.id, now=STAMP)

        pipeline = StatusPipeline([])
        updated = _status.set_subtree_status(pipeline, graph_conn(graph), goal.id, NodeStatus.DONE)
    ...
    def test_fully_archived_subtree_stamps_nothing_and_returns_zero(
        ...
        goal, left, right = _done_goal_with_two_tasks(graph)
        _ = _mutations.archive_subtree(graph_conn(graph), goal.id, now=STAMP)
        pipeline = StatusPipeline([])
        blocked = _status.set_subtree_status(
            pipeline, graph_conn(graph), goal.id, NodeStatus.BLOCKED
        )
        assert blocked == 0
        for nid in (goal.id, left.id, right.id):
    ...
    def test_cascade_to_done_releases_goal_claim(self, graph: MikadoGraph) -> None:
        ...
        _ = graph.add_node("task", parent_id=goal.id)
        assert graph.claim_or_reclaim_goal(goal.id, "run-a", now="2026-01-01T00:00:00+00:00")

        pipeline = StatusPipeline([])
        _ = _status.set_subtree_status(pipeline, graph_conn(graph), goal.id, NodeStatus.DONE)
    ...
    def test_cascade_to_pending_clears_worktree_and_run_pins(self, graph: MikadoGraph) -> None:
        ...
        pipeline = StatusPipeline([])
        _ = _status.set_subtree_status(pipeline, graph_conn(graph), node.id, NodeStatus.PENDING)
    ...
    def test_missing_root_raises(self, graph: MikadoGraph) -> None:
        pipeline = StatusPipeline([])
        with pytest.raises(ValueError, match="Node 999 not found"):
            _ = _status.set_subtree_status(pipeline, graph_conn(graph), 999, NodeStatus.DONE)
    ...
    def test_illegal_transition_mid_subtree_writes_nothing(self, graph: MikadoGraph) -> None:
        ...
        with pytest.raises(InvalidTransition):
            _ = _status.set_subtree_status(
                pipeline, graph_conn(graph), goal.id, NodeStatus.PENDING
            )
...
class TestAddNodeUnderArchivedParent:
    def test_add_node_rejects_archived_parent(self, graph: MikadoGraph) -> None:
        ...
        _ = _mutations.archive_subtree(graph_conn(graph), parent.id)
        with pytest.raises(
            ValueError,
            match=f"Cannot add a node under archived parent {parent.id}; unarchive it first.",
        ):
            _ = graph.add_node("child", parent_id=parent.id)
    ...
    def test_move_node_rejects_archived_parent(self, graph: MikadoGraph) -> None:
        ...
        incoming = (
            graph_conn(graph)
            .execute("SELECT parent_id FROM edges WHERE child_id = ?", (mover.id,))
            .fetchall()
        )
        assert incoming == []
    ...
    def test_add_node_under_archived_descendant_rejected(self, graph: MikadoGraph) -> None:
        ...
        with pytest.raises(
            ...
            _ = graph.add_node("grandchild", parent_id=child.id)
    ...
    def test_add_node_allowed_after_unarchive(self, graph: MikadoGraph) -> None:
        ...
        _complete(graph, parent.id)
        _ = _mutations.archive_subtree(graph_conn(graph), parent.id)
        _ = _mutations.unarchive_subtree(graph_conn(graph), parent.id)
        child = graph.add_node("child", parent_id=parent.id)
...
class TestFencedWritesOnArchivedNode:
    ...
    def test_fenced_writes_on_archived_node_are_safe_noops(self, graph: MikadoGraph) -> None:
        node = graph.add_node("done")
        _complete(graph, node.id)
        _ = _mutations.archive_subtree(graph_conn(graph), node.id, now=STAMP)
...
class TestMigrationV3:
    def test_archived_at_persists_across_reopen(self, tmp_path: Path) -> None:
        ...
        node = graph.add_node("done")
        _complete(graph, node.id)
        assert _mutations.archive_subtree(graph_conn(graph), node.id, now=STAMP) == 1
        graph.close()
        ...
        try:
            ...
            assert node.archived_at == datetime.fromisoformat(STAMP)
            version = cast(int, graph_conn(reopened).execute("PRAGMA user_version").fetchone()[0])
