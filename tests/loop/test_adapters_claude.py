...
def test_extract_completion_signal_from_result_text() -> None:
    ...
    assert (
        adapter.extract_completion_signal(result_text=result_text, stdout=None, user_signal="DONE")
        is True
    )
    assert (
...
def test_extract_completion_signal_ignores_raw_stdout() -> None:
    ...
    assert (
        adapter.extract_completion_signal(result_text=None, stdout=stdout, user_signal="MARKER")
        is False
    )
...
def test_extract_completion_signal_handles_missing_result_text() -> None:
    adapter = ClaudeAdapter()
    assert (
        adapter.extract_completion_signal(result_text=None, stdout=None, user_signal="DONE")
        is False
    )
    assert (
        adapter.extract_completion_signal(result_text="", stdout=None, user_signal="DONE") is False
    )
