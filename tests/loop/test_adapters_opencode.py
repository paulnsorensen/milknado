...
def test_extract_completion_signal_from_stdout() -> None:
    ...
    assert (
        adapter.extract_completion_signal(result_text=None, stdout=stdout, user_signal="DONE")
        is True
    )
    assert (
        adapter.extract_completion_signal(result_text=None, stdout=stdout, user_signal="OTHER")
        is False
    )
