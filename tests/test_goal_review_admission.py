...
def test_goal_review_functions_open_own_transaction_when_standalone(tmp_path: Path) -> None:
    graph, nodes = _hierarchy(tmp_path)
    try:
        ...
        record = _goal_review.request_goal_review(
            ...
            GoalReviewRequest(
                goal_id=nodes["goal_a"],
                goal_revision="sha256:goal-contract",
                evidence="Standalone evidence entry.",
                proposed_change="Standalone proposed change.",
        ...
        assert record.decision is GoalReviewDecision.PENDING
        decided = _goal_review.decide_goal_review(
            conn,
            GoalReviewDecisionRequest(
                review_id=record.review_id, decision=GoalReviewDecision.ACCEPTED
            ),
            decided_by="human-1",
        )
        assert decided.decision is GoalReviewDecision.ACCEPTED
...
def test_enqueue_goal_review_interrupts_standalone_requires_existing_review(
    tmp_path: Path,
) -> None:
    graph, _ = _hierarchy(tmp_path)
    try:
        with pytest.raises(ValueError, match="goal review 999 not found"):
            _ = _review_interrupts.enqueue_goal_review_interrupts(graph_conn(graph), 999)
    finally:
        graph.close()


def test_complete_root_returns_false_without_eligible_root(tmp_path: Path) -> None:
    graph = MikadoGraph(tmp_path / "graph.db")
    try:
        assert graph.complete_root() is False
    finally:
        graph.close()


def test_set_todo_status_rejects_missing_node(tmp_path: Path) -> None:
    graph, _ = _hierarchy(tmp_path)
    try:
        with pytest.raises(ValueError, match="Node 99999 not found"):
            _ = graph.set_todo_status(99999, NodeStatus.DONE)
    finally:
        graph.close()
