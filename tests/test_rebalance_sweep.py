...
class TestSweep:
    def test_archives_all_done_root_subtree(self, conn: sqlite3.Connection) -> None:
        root = insert_node(conn, "done goal", "done", kind="goal")
        child = insert_node(conn, "done task", "done", parent_id=root)
        assert sweep_archivable(conn) == [root]
        assert archived_ids(conn) == {root, child}

    def test_skips_mixed_status_subtree(self, conn: sqlite3.Connection) -> None:
        root = insert_node(conn, "goal", "done", kind="goal")
        _ = insert_node(conn, "pending task", "pending", parent_id=root)
        assert sweep_archivable(conn) == []
        assert archived_ids(conn) == set()

    def test_skips_pending_root(self, conn: sqlite3.Connection) -> None:
        _ = insert_node(conn, "pending root", "pending")
        assert sweep_archivable(conn) == []

    def test_skips_already_archived_root(self, conn: sqlite3.Connection) -> None:
        _ = insert_node(conn, "archived", "done", archived_at=NOW)
        assert sweep_archivable(conn) == []

    def test_claim_does_not_hide_done_subtree_from_sweep(self, conn: sqlite3.Connection) -> None:
        root = insert_node(conn, "claimed goal", "done", kind="goal")
        _ = conn.execute(
            "INSERT INTO goal_claims (goal_id, run_id, pid, claimed_at) VALUES (?, ?, ?, ?)",
            (root, "run-x", 12345, NOW),
        )
        assert sweep_archivable(conn) == [root]
        assert archived_ids(conn) == {root}

    def test_archives_only_eligible_roots(self, conn: sqlite3.Connection) -> None:
        done_root = insert_node(conn, "done", "done", kind="goal")
        _ = insert_node(conn, "live", "in_progress", kind="goal")
        assert sweep_archivable(conn) == [done_root]

    def test_nested_done_subtree_archives_maximal_root_only(
        self, conn: sqlite3.Connection
    ) -> None:
        root = insert_node(conn, "done root goal", "done", kind="goal")
        child_goal = insert_node(conn, "done child goal", "done", kind="goal", parent_id=root)
        grandchild = insert_node(conn, "done leaf", "done", parent_id=child_goal)
        # Exactly one archived root — the maximal one, never children separately.
        assert sweep_archivable(conn) == [root]
        assert archived_ids(conn) == {root, child_goal, grandchild}

    def test_shared_done_descendant_is_not_selected_as_a_root(
        self, conn: sqlite3.Connection
    ) -> None:
        left = insert_node(conn, "left", "done", kind="goal")
        right = insert_node(conn, "right", "done", kind="goal")
        shared = insert_node(conn, "shared", "done", parent_id=left)
        ...
        assert archived_ids(conn) == {left, right, shared}
    ...
    def test_nested_maximal_subtree_under_live_ancestor_is_archived(
        ...
        roadmap = insert_node(conn, "live roadmap", "in_progress", kind="goal")
        done_goal = insert_node(conn, "done goal", "done", kind="goal", parent_id=roadmap)
        leaf = insert_node(conn, "done task", "done", parent_id=done_goal)
        live_sibling = insert_node(conn, "live task", "pending", parent_id=roadmap)
        assert sweep_archivable(conn) == [done_goal]
        assert archived_ids(conn) == {done_goal, leaf}
        assert live_sibling not in archived_ids(conn)
        assert roadmap not in archived_ids(conn)
    ...
    def test_find_archivable_roots_is_read_only(self, conn: sqlite3.Connection) -> None:
        ...
        assert archived_ids(conn) == set()  # selection never stamps

    def test_partially_archived_subtree_stamps_only_unarchived(
        self, conn: sqlite3.Connection
    ) -> None:
        stale_stamp = "2020-01-01T00:00:00+00:00"
        root = insert_node(conn, "done root", "done", kind="goal")
        pre_archived = insert_node(
            conn, "already archived", "done", parent_id=root, archived_at=stale_stamp
        )
        fresh = insert_node(conn, "not yet archived", "done", parent_id=root)
        assert sweep_archivable(conn) == [root]
        assert archived_ids(conn) == {root, pre_archived, fresh}
    ...
    def test_claimed_descendant_does_not_block_sweep(self, conn: sqlite3.Connection) -> None:
        root = insert_node(conn, "done root", "done", kind="goal")
        claimed_child = insert_node(conn, "done child goal", "done", kind="goal", parent_id=root)
        _ = conn.execute(
            "INSERT INTO goal_claims (goal_id, run_id, pid, claimed_at) VALUES (?, ?, ?, ?)",
            (claimed_child, "run-nested", 4242, NOW),
        )
        assert sweep_archivable(conn) == [root]
        assert archived_ids(conn) == {root, claimed_child}
    ...
    def test_claimed_nested_subtree_under_live_ancestor_is_archived(
        ...
        assert sweep_archivable(conn) == [done_goal]
        assert archived_ids(conn) == {done_goal}
...
class TestGroupOrphans:
    def test_creates_inbox_and_attaches_orphans(self, conn: sqlite3.Connection) -> None:
        orphan = insert_node(conn, "bare task", "pending")
    ...
    def test_idempotent_inbox_creation(self, conn: sqlite3.Connection) -> None:
        _ = insert_node(conn, "orphan a", "pending")
        assert group_orphans(conn) == 1
    ...
    def test_does_not_reattach_parented_nodes(self, conn: sqlite3.Connection) -> None:
        goal = insert_node(conn, "goal", "in_progress", kind="goal")
        _ = insert_node(conn, "child", "pending", parent_id=goal)
        assert group_orphans(conn) == 0

    def test_skips_archived_orphans(self, conn: sqlite3.Connection) -> None:
        _ = insert_node(conn, "archived orphan", "done", archived_at=NOW)
        assert group_orphans(conn) == 0

    def test_skips_claimed_orphan_subtree(self, conn: sqlite3.Connection) -> None:
        goal = insert_node(conn, "claimed orphan goal", "in_progress", kind="goal")
        _ = conn.execute(
            "INSERT INTO goal_claims (goal_id, run_id, pid, claimed_at) VALUES (?, ?, ?, ?)",
            (goal, "run-c", 999, NOW),
        )
    ...
    def test_inbox_itself_is_never_moved(self, conn: sqlite3.Connection) -> None:
        inbox_id = insert_node(conn, INBOX_DESCRIPTION, "pending", kind="goal")
    ...
    def test_dangling_parent_id_is_not_treated_as_an_orphan(
        ...
        parent = insert_node(conn, "doomed parent", "done", kind="goal")
        child = insert_node(conn, "task", "pending", parent_id=parent)
    ...
    def test_task_under_archived_parent_is_not_a_root_orphan(
        ...
        parent = insert_node(conn, "archived parent", "done", kind="goal", archived_at=NOW)
        child = insert_node(conn, "live task under archive", "pending", parent_id=parent)
        assert group_orphans(conn) == 0
        row = cast(
            sqlite3.Row,
            conn.execute("SELECT parent_id FROM nodes WHERE id = ?", (child,)).fetchone(),
    ...
    def test_inbox_named_task_does_not_count_as_inbox(self, conn: sqlite3.Connection) -> None:
        ...
        impostor = insert_node(conn, INBOX_DESCRIPTION, "pending", kind="task")
        orphan = insert_node(conn, "real orphan", "pending")
    ...
    def test_archived_inbox_is_never_reused_for_live_orphans(
        ...
        archived_inbox = insert_node(conn, INBOX_DESCRIPTION, "done", kind="goal", archived_at=NOW)
        orphan = insert_node(conn, "live orphan", "pending")
        assert group_orphans(conn) == 1
        row = cast(
            sqlite3.Row,
            conn.execute("SELECT parent_id FROM nodes WHERE id = ?", (orphan,)).fetchone(),
    ...
    def test_zero_orphans_creates_no_inbox(self, conn: sqlite3.Connection) -> None:
        ...
        goal = insert_node(conn, "live goal", "in_progress", kind="goal")
        _ = insert_node(conn, "parented child", "pending", parent_id=goal)
        assert group_orphans(conn) == 0
        inbox = cast(
            sqlite3.Row | None,
            conn.execute(
